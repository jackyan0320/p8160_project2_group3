...
cv <- function(org_data, lambdas, K){ #original data
  ...
  for(i in 1:K){
    ...
    expu <- exp(u) 
    prob <- expu / (1 + expu) 
    cv_errors[i] = mean((testData$y-prob)^2) #MSE
  }
  cv_error[j] <- mean(cv_errors)
  cv_se[j] <- sqrt(var(cv_errors)/K)
  }
  return(cbind(lambdas, cv_error, cv_se))
}
...
```{r}
best_lambda <- cv_res[which.min(cv_res[,2]),1]
cv_res <- data.frame(cv_res)
cv_res %>% 
  filter(lambdas == best_lambda)

cv_res %>% 
    ggplot(aes(x = log(lambdas), y=cv_error)) + 
    ...
    annotate("text", log(best_lambda), 0.05, label = paste("best log(lambda) = ", round(log(best_lambda), 3), sep = ""))
